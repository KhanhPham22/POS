...
public class SupplierPanel extends JPanel {
	...
	private void initUI() {
		...
		btnNextItem.addActionListener(e -> {
			// Lấy tổng số items để tính số trang tối đa
		...
		itemTableModel = new DefaultTableModel(
			...
			public boolean isCellEditable(int row, int column) {
				// Chỉ cho phép chỉnh sửa trên cột "Thao tác" (cột thứ 4, index 4)
				return column == 4;
			}
		};
		...
		itemTable.setRowHeight(30); // Chiều cao mỗi hàng là 30px (có thể điều chỉnh)

		// Đặt chiều cao tổng thể của bảng để vừa đủ 10 hàng + tiêu đề
		int rowHeight = 30; // Chiều cao mỗi hàng
		int headerHeight = itemTable.getTableHeader().getPreferredSize().height; // Chiều cao tiêu đề
		int totalHeight = headerHeight + (rowHeight * itemPageSize); // Chiều cao tổng = tiêu đề + 10 hàng
		itemTable.setPreferredSize(new Dimension(itemTable.getPreferredSize().width, totalHeight));

		// Đặt kích thước cho JScrollPane chứa bảng
		JScrollPane itemTableScrollPane = new JScrollPane(itemTable);
		itemTableScrollPane
				.setPreferredSize(new Dimension(itemTableScrollPane.getPreferredSize().width, totalHeight + 5)); // +5
																													// để
																													// tránh
																													// cắt
																													// mất
		...
		public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
			...
			if (value instanceof Supplier) {
				...
				setText("<html><b>" + supplier.getName() + "</b><br>" +
				        "Người liên hệ: " + (supplier.getContactName() != null ? supplier.getContactName() : "(Chưa có)") + "<br>" +
				        "Điện thoại: " + (supplier.getPhone() != null ? supplier.getPhone() : "(Chưa có)") + "<br>" +
				        "Email: " + (supplier.getEmail() != null ? supplier.getEmail() : "(Chưa có)") + "<br>" +
				        "Mã số thuế: " + (supplier.getTaxCode() != null ? supplier.getTaxCode() : "(Chưa có)") + "<br>" +
	...
	private static class ButtonEditor extends DefaultCellEditor {
		...
		public ButtonEditor(JCheckBox checkBox, ItemService itemService, SupplierPanel supplierPanel) {
			...
			button.addActionListener(e -> {
				if (currentItem != null) {
					System.out.println("Editing item: ID=" + currentItem.getId() + ", Name=" + currentItem.getName());
					ItemPanel itemPanel = new ItemPanel(currentItem, supplierPanel.currentSupplier, itemService,
							supplierPanel);
	...
	private void loadSuppliers() {
		...
		if (suppliers == null || suppliers.isEmpty()) {
			...
			btnNextSupplier.setEnabled(suppliers.size() == supplierPageSize);
		}
	...
	private void searchSuppliers(String query) {
		...
		lblSupplierPageInfo.setText(
				"Page " + supplierPageNumber + " of " + ((suppliers.size() + supplierPageSize - 1) / supplierPageSize));
		btnPrevSupplier.setEnabled(supplierPageNumber > 1);
	...
	private void loadItemTable(Supplier supplier) {
		...
		// Lấy tổng số items của supplier để tính số trang
		List<Item> allItems = itemService.getItemsBySupplierId(supplier.getId(), 1, Integer.MAX_VALUE); // Lấy tất cả
																										// items
		int totalItems = allItems.size();
		int totalPages = (int) Math.ceil((double) totalItems / itemPageSize);
		...
		btnPrevItem.setEnabled(itemPageNumber > 1);
		btnNextItem.setEnabled(itemPageNumber < totalPages); // Chỉ bật nút "Next" nếu chưa phải trang cuối
	}
